...
export interface PageFlipViewerProps {
  /** The keys to display, in order — usually the 1200px web variants. */
  keys: string[]
  /** Full-size keys to fall back to when a web variant is missing. */
  masterKeys?: string[]
  /** For alt text and the fullscreen dialog label. */
  title: string
  /** Extra controls beneath the pager, given the current 0-based frame. */
  controls?: (i: number) => ReactNode
  /** Extra controls in the fullscreen header; `close` dismisses the overlay. */
  fullscreenControls?: (i: number, close: () => void) => ReactNode
}

/**
 * The portal's one way of reading a page set: a single page, click for full
 * screen, arrows / swipe / trackpad to turn, prefetching either side.
 *
 * Extracted from ComicReader so the EDITABLE DECK reads exactly like the comic.
 * The deck reader first embedded its PDF in an iframe — the same content, but
 * with the browser's own PDF chrome, scrollbar and zoom sitting inside a page
 * whose reader pages through images. Adnan: "the editable deck system UI is
 * very different from the read-the-comic UI. It should be the same." One
 * component is how that stays true the next time either changes.
 *
 * Callers supply keys; what those keys point at is their business — comic
 * pages, deck slides, anything paged.
 */
export function PageFlipViewer({
  keys, masterKeys, title, controls, fullscreenControls,
}: PageFlipViewerProps) {
  // A web variant that 404s on a fresh presign falls back to its master.
  const masters = masterKeys ?? keys
  const [masterFallback, setMasterFallback] = useState<ReadonlySet<number>>(
    () => new Set<number>(),
  )
  const shown = keys.map((k, idx) => (masterFallback.has(idx) ? masters[idx] : k))
  const urls = useResolved(shown)
  const total = shown.length
  const [i, setI] = useState(0)
  const [fs, setFs] = useState(false)

  // Turning the page should be instant, so warm the neighbours. The browser
  // dedupes repeat loads; the ref set just avoids re-kicking known URLs.
  const prefetched = useRef<Set<string>>(new Set())
  const currentUrl = urls[shown[i]]
  useEffect(() => {
    for (const j of [i + 1, i + 2, i + 3, i - 1]) {
      if (j < 0 || j >= shown.length) continue
      const url = urls[shown[j]]
  ...
  // A page set can change under the viewer (a language switch): start over
  // rather than stranding the reader past the end of a shorter edition.
  const signature = keys.join('|')
  useEffect(() => {
    setI(0)
    setMasterFallback(new Set<number>())
    erroredOnce.current = new Set<string>()
  }, [signature])
          ...
          >
            <div className="flex items-center justify-between px-5 py-3 text-white/80">
              ...
              <div className="flex items-center gap-2">
                {fullscreenControls?.(i, () => setFs(false))}
                <button
                  type="button"
                  onClick={() => setFs(false)}
